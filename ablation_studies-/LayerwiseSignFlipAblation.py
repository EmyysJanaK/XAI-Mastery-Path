...
class LayerwiseSignFlipAblation:
    ...
    def visualize_results(self, save_path: Optional[str] = None) -> None:
        ...
        # Plot SER and SID accuracy drops
        plt.plot(layer_indices, ser_drops, 'b-', marker='o', linewidth=2, label='SER Accuracy Drop')
        plt.plot(layer_indices, sid_drops, 'r-', marker='s', linewidth=2, label='SID Accuracy Drop')
        
        # Add shaded regions for hypothesized layers
        plt.axvspan(4, 10, alpha=0.2, color='blue', label='Hypothesized SER region (layers 5-10)')
        plt.axvspan(10, 12, alpha=0.2, color='red', label='Hypothesized SID region (layers 11-12)')
        
        # Set labels and title
        plt.xlabel('Layer Index', fontsize=14)
        plt.ylabel('Accuracy Drop (Baseline - Ablated)', fontsize=14)
        plt.title('Layer-wise Sign-Flipping Ablation Study', fontsize=16)
        ...
        plt.annotate(f'Max SER Drop: {ser_drops[max_ser_idx]:.4f}',
                    xy=(max_ser_idx, ser_drops[max_ser_idx]),
                    xytext=(max_ser_idx, ser_drops[max_ser_idx] + 0.05),
                    arrowprops=dict(facecolor='black', shrink=0.05),
                    fontsize=12)
        
        plt.annotate(f'Max SID Drop: {sid_drops[max_sid_idx]:.4f}',
